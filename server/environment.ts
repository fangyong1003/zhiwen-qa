...
export function readEnvironment(values: NodeJS.ProcessEnv) {
  return z.object({
    ...
    CHAT_CONTEXT_TOKENS: z.preprocess(emptyToUndefined, z.coerce.number().int().min(16000).max(64000).default(24000)),
    ...
    DEEPSEEK_BASE_URL: z.preprocess(emptyToUndefined, z.url().refine((value) => ["http:", "https:"].includes(new URL(value).protocol), "DEEPSEEK_BASE_URL must be an HTTP(S) URL").optional()),
